%% iz_force_movie_split_regression_test: FORCE-train a spiking NN on fMRI data.
% Adapted from
...
data_eye = eye(num_data_dims,num_data_dims);
data_mask = reshape(  repmat( reshape(data_eye,1,[]), neurons_per_sc_node, 1 ), [], num_data_dims  );
[iz_scalar_consts,iz_matrix_consts,iz_state] = define_iz_neural_network_split(num_data_dims,num_hdts_dims,'N',N,'data_mask',data_mask,'BPhi1_mask',data_mask');
...
model_name = sprintf('iz_force_N_%u_SepWeights2_subject_%s_ts_%u_multiplier_%u_blocks_%u_reps_%u_start_ntb_%u_end_ntb_%u',N,subject_id,ts_index,num_interpolated_steps_per_original_step,num_blocks,num_training_repeats,num_non_training_blocks_at_start,num_non_training_blocks_at_end);
...
for repeat_index = 1:num_training_repeats
    for block_index = 1:num_blocks
        ...
        for step_index = 1:num_steps_in_block
            iz_state = update_iz_neurons_split( iz_state, iz_matrix_consts, iz_scalar_consts, S.interpolated_hdts_block(:,step_index) );
            ...
            if is_training_block
                iz_state = rls_update_split(iz_state,iz_matrix_consts);
            end
            current_time = toc;
...
for repeat_index = 1:num_testing_repeats
    for block_index = 1:num_blocks
        ...
        for step_index = 1:num_steps_in_block
            iz_state = update_iz_neurons_split( iz_state, iz_matrix_consts, iz_scalar_consts, S.interpolated_hdts_block(:,step_index) );
            model_output_for_block(:,step_index) = iz_state.z1;
            iz_state.err = iz_state.z1 - S.interpolated_data_block(:,step_index);
            sq_err = iz_state.err.^2;
            sq_err_of_block(step_index,:) = sq_err;
            mse_over_areas = mean( sq_err, 'all' );
