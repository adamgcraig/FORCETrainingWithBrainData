%% iz_force_movie_split4_regression_test: FORCE-train a spiking NN on fMRI data.
% Adapted from
...
num_training_repeats = 60;% number of times to repeat each block during training
num_non_training_steps_before = 0;
num_non_training_steps_after = round(0.5*num_data_time_points);
...
randomize = false;
internal_weights = scale_network_simple_ei(sc_data,nodes_per_region,randomize);

%% Make fixed weights so that each region only receives its own data.

data_weights = make_data_weights_by_region(num_regions,nodes_per_region);
...
model_name = sprintf('iz_force_split4_sclike_rand_%u_N_%u_subject_%s_ts_%u_data_dt_%g_model_dt_%g_nt_pre_%u_post_%u_reps_%u', ...
    randomize,num_internal_nodes,subject_id,ts_index,data_dt,model_dt,num_non_training_steps_before,num_non_training_steps_after,num_training_repeats);
...
[~, ~] = update_iz_neurons_multiple_steps( supervisor_data(:,1), supervisor_hdts(:,1), 0, false, 'dt', model_dt, 'internal_weights', internal_weights, 'data_weights', data_weights );% 'N', num_internal_nodes );
